...
library UniswapV2Library {
  ...
  function getAmountsOut(address factory, uint256 amountIn, address[] calldata path) public returns (uint256[] memory) {
  ...
  function getAmountsIn(address factory, uint256 amountOut, address[] calldata path) public returns (uint256[] memory) {
    if (path.length < 2) revert InvalidPath();
    uint256[] memory amounts = new uint256[](path.length);
    amounts[path.length - 1] = amountOut;

    for (uint256 i = path.length - 1; i > 0; i -= 1) {
      (uint256 reserveIn, uint256 reserveOut) = getReserves(factory, path[i], path[i - 1]);
